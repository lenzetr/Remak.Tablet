...
namespace Remak.Tablet.UI.Database
{
    public class SQLLite
    {
        ...
        public string DatabaseName => Path.Combine(Global.DatabaseDirectory, $"DatabaseLite.db3");
        ...
        public bool CheckTable(string tableName)
        {
            ...
            using (var cmd = new SQLiteCommand())
            {
                ...
                try
                {
                    ...
                    var result = sh.ExecuteScalar<string>("SELECT tbl_name FROM sqlite_master WHERE type = 'table' AND tbl_name = @TableName",
                        new[] { new SQLiteParameter("@TableName", tableName) });
